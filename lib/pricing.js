 ...
 * FIXED: County detection logic - Emergency fix deployed Aug 19, 2025 8:15 PM
 */
...
export async function checkFranklinCountyStatus(pickupAddress, destinationAddress) {
  try {
    ...
    console.log('🚨 DISPATCHER COUNTY DETECTION EMERGENCY CHECK 🚨', { pickup, destination });
    ...
    if (isPickupFranklin && isDestinationFranklin) {
      console.log('🚨 DISPATCHER EMERGENCY FIX APPLIED: Both addresses detected as Franklin County');
      return {
 ...
 * FACILITY CLIENTS: Use specified rate structure ($50/leg, $3-4/mile, surcharges)
 * INDIVIDUAL CLIENTS: Use booking app rate structure (may differ)
 */
export function calculateTripPrice({
  ...
  // FACILITY CLIENTS: No wheelchair fees for facility clients
  if (wheelchairType === 'provided') {
    if (clientType === 'facility') {
      breakdown.wheelchairPrice = 0; // No wheelchair fee for facility clients
    } else {
      breakdown.wheelchairPrice = PRICING_CONFIG.PREMIUMS.WHEELCHAIR_RENTAL; // $25 for individual clients
    }
  }
